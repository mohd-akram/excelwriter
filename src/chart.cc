...
Napi::Value Chart::SetTitleNameFont(const Napi::CallbackInfo& info) {
  ...
  if (!size.IsUndefined()) font.size = size.As<Napi::Number>();
  if (!bold.IsUndefined())
    font.bold = bold.As<Napi::Boolean>() ? LXW_TRUE : LXW_EXPLICIT_FALSE;
  if (!italic.IsUndefined()) font.italic = italic.As<Napi::Boolean>();
  if (!underline.IsUndefined()) font.underline = underline.As<Napi::Boolean>();
  if (!rotation.IsUndefined()) font.rotation = rotation.As<Napi::Number>();
  if (!color.IsUndefined()) font.color = color.As<Napi::Number>();
  if (!pitchFamily.IsUndefined())
